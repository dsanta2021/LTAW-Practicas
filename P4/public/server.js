...
io.on('connect', (socket) => {
  ...
  socket.on('message', ({ room, message }) => {
    ...
    sendToElectron({ type: 'message', username, message, room });
...
//-- Funcionalidad con Electron
function sendToElectron(data) {
  if (process.send) process.send(data);
}

//-- Cuando cambia el número de usuarios conectados
function updateUserCount() {
  sendToElectron({ type: 'userCount', count: connectedUsers });
}
io.on('connect', (socket) => {
  // ...tu código...
  updateUserCount();
...
//-- Recibe mensaje de prueba desde Electron
process.on('message', (msg, room) => {
  if (msg.type === 'test-message') {
    io.emit('serverMessage', { msg: `MENSAJE DE PRUEBA DESDE EL SERVIDOR`, room });
  }
});
